...
";
        ...
        public static void AssertBaseline(params string[] expectedFragments)
        {
            ...
            try
            {
                Assert.Equal(sqlStatements[0], expectedFragments[0]);

                foreach (var expectedFragment in expectedFragments.Skip(1))
                {
            ...
            AssertSql(
                {string.Join("," + indent + "//" + indent, sqlStatements.Take(9).Select(sql => "@\"" + sql + "\""))});
